...
def play_card(player,opponent,move):
    ...
    if crd.special:
        ...
        if crd.value == 11:
            ...
            cards_to_remove, protected = rules.get_cards_removed_by_joker(player,opponent,move)
        elif crd.value == 13:
            rules.double_total_with_king(move)
        for card in cards_to_remove:
            next(c for c in player.caravans + opponent.caravans if card in c.cards).cards.remove(card)
