...
class GameInterface():
    ...
    def play_cards(self):
        while True:
            ...
            if move is None:
                break
            caravan = move[0]
            is_allowed = rules.check_if_legal_move(self.player,self.opponent,move)
            if not is_allowed[0]:
                print(is_allowed[1])
            ...
            if crd.special:
                ...
                if crd.value == 11:
                    ...
                    cards_to_remove = rules.get_cards_removed_by_joker(self.player,self.opponent,move)
                elif crd.value == 13:
    ...
    def game_loop(self):
        while rules.is_player_winner(self.player,self.opponent) is None:
            self.clear_screen()
            input('press any key to start your turn Player '+str(self.pl_nums[0]))
            self.print_setup()
