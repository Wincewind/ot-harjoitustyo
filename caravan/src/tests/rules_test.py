...
class TestRules(unittest.TestCase):
    ...
    def test_check_putting_card_into_wrong_deck_when_all_own_caravans_not_started(self):
        ...
        self.assertEqual(rules.check_if_legal_move(
            self.player, self.opponent, move)[0], False)

        # Opponent puts card into your caravan when own not started
        move = (self.player.caravans[1], 0, self.opponent.hand[0])
        self.assertEqual(rules.check_if_legal_move(
            self.opponent, self.player, move)[0], False)
    ...
    def test_check_correct_caravan_starting(self):
        ...
        self.assertEqual(rules.check_if_legal_move(
            self.player, self.opponent, move), (True, ''))
        ...
        self.assertEqual(rules.check_if_legal_move(
            self.player, self.opponent, move)[0], True)
    ...
    def test_putting_cards_into_opponent_caravan(self):
        ...
        self.assertEqual(rules.putting_card_into_opponent_caravan(
    ...
    def test_jack_effect(self):
        ...
        move = (self.opponent.caravans[0], 5, Card(None, 'Spades', 11, True))
        # self.assertEqual(all(True if c in rules.get_cards_removed_by_jack(move) else False for c in [c5, c4, c3, c2]), True)
        self.assertEqual(rules.get_cards_removed_by_jack(
            move), [c5, c4, c3, c2])
    ...
    def test_joker_effect(self):
        ...
        self.assertEqual(set(rules.get_cards_removed_by_joker(
            self.player, self.opponent, move)[0]), {s2, s3, s4, s5, sq})
        move = (self.player.caravans[2], 1, Card(None, 'Red', 0, True))
        self.assertEqual(set(rules.get_cards_removed_by_joker(
            self.player, self.opponent, move)[0]), {s2, d2, h21, sq})

    def test_king_effect(self):
        s2 = Card(None, 'Spades', 2, False)
        s5 = Card(None, 'Spades', 5, False)
        hk = Card(None, 'Hearts', 13, True)
        sq = Card(None, 'Spades', 12, True)
        self.player.caravans[0].insert_card(s2)
        ...
        rules.double_total_with_king(move)
        self.assertEqual(self.player.caravans[0].value, 14)
    ...
    def test_winner(self):
        ...
        s5 = Card(None, 'Spades', 5, False)
        h4 = Card(None, 'Hearts', 4, False)
        s1 = Card(None, 'Spades', 1, False)
        hk = Card(None, 'Hearts', 13, True)
        hk = Card(None, 'Hearts', 13, True)
        sq = Card(None, 'Spades', 12, True)
        self.player.caravans[0].insert_card(copy.copy(s10))
        self.player.caravans[1].insert_card(copy.copy(s10))
        self.player.caravans[2].insert_card(copy.copy(s10))
        ...
        rules.double_total_with_king(move)
        move = (self.player.caravans[2], 1, hk)
        rules.double_total_with_king(move)
        self.player.caravans[0].insert_card(s5)
        ...
        rules.double_total_with_king(move)
        move = (self.opponent.caravans[2], 1, hk)
        rules.double_total_with_king(move)
