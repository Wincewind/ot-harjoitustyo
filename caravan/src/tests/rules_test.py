...
class TestRules(unittest.TestCase):
    ...
    def test_joker_effect(self):
        ...
        self.assertEqual(set(rules.get_cards_removed_by_joker(
            self.player, self.opponent, move)), {s2, s3, s4, s5, sq})
        move = (self.player.caravans[2], 0, Card(None, 'Red', 0, True))
        self.assertEqual(set(rules.get_cards_removed_by_joker(
            self.player, self.opponent, move)), {s2, d2, h21, sq})
    ...
    def test_king_effect(self):
        ...
        move = (self.player.caravans[0], 0, hk)
        rules.double_total_with_king(move)
    ...
    def test_winner(self):
        ...
        move = (self.player.caravans[0], 0, hk)
        rules.double_total_with_king(move)
        move = (self.player.caravans[1], 0, hk)
        rules.double_total_with_king(move)
        move = (self.player.caravans[2], 0, hk)
        rules.double_total_with_king(move)
        self.player.caravans[0].insert_card(s5)
        self.player.caravans[1].insert_card(s5)
        self.player.caravans[2].insert_card(s5)
        self.assertEqual(rules.is_player_winner(self.player,self.opponent), True)
        move = (self.opponent.caravans[0], 0, hk)
        rules.double_total_with_king(move)
        move = (self.opponent.caravans[1], 0, hk)
        rules.double_total_with_king(move)
        move = (self.opponent.caravans[2], 0, hk)
        rules.double_total_with_king(move)
